...
from trailtag.tools.redis_cache_tool import RedisCacheTool
...
def run(url: str = None):
    ...
    if not url:
        print("No YouTube video URL provided.")
        return
    ...
    try:
        ...
        youtube_url = inputs["youtube_video_url"]
        redis_tool = RedisCacheTool(prefix="trailtag:map_routes:")
        cached_result = redis_tool.get(youtube_url)
        if cached_result:
            print(f"<<< 已找到 {youtube_url} 的地圖路線快取，直接回傳，不執行 crew。")
            print(f"Map routes result (from cache): {cached_result}")
            return
