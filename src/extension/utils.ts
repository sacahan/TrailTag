...
export type MapVisualization = { routes?: Route[] };
 ...
 */
export function extractVideoId(url: string | null): Maybe<string> {
  if (!url) return null;
    ...
    if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
      return new Promise((resolve) => {
        try {
          ...
          try { localStorage.setItem('trailtag_state_v1', JSON.stringify(payload)); } catch {};
          resolve();
        }
      });
    }
    // fallback to localStorage
    try { localStorage.setItem('trailtag_state_v1', JSON.stringify(payload)); } catch (e) { }
  } catch (e) { }
