...
const MAX_RECONNECT = (typeof globalThis.TRAILTAG_CONFIG !== 'undefined' && globalThis.TRAILTAG_CONFIG?.MAX_RECONNECT != null)
    ? globalThis.TRAILTAG_CONFIG.MAX_RECONNECT
    : 1;
/**
...
async function listenToJobEvents(jobId) {
    ...
    try {
        ...
        if (typeof globalThis !== 'undefined' && globalThis.TRAILTAG_CONFIG) {
            Object.assign(resolvedConfig, globalThis.TRAILTAG_CONFIG);
        }
        if (storageVal) {
    ...
    const API_BASE_URL = (resolvedConfig && resolvedConfig.API_BASE_URL) || 'http://localhost:8010';
    ...
    eventSource.addEventListener('phase_update', (event) => {
        try {
            ...
            if (reconnectAttempts[jobId])
                reconnectAttempts[jobId] = 0;
    ...
    eventSource.addEventListener('error', (event) => {
        ...
        if (attempts < MAX_RECONNECT) {
            ...
            // 計算重試延遲（指數退避 + 隨機抖動）
            const baseDelay = (typeof globalThis.TRAILTAG_CONFIG !== 'undefined' && globalThis.TRAILTAG_CONFIG?.FETCH_BACKOFF_MS)
                ? globalThis.TRAILTAG_CONFIG.FETCH_BACKOFF_MS
                : 800;
