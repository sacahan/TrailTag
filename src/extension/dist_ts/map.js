 ...
 */
// 模組等級的單例變數
export let leafletMap = null; // Leaflet 地圖實例（單例）
export let markersLayer = null; // 所有標記匯集的 layer（featureGroup 或 layerGroup）
export let currentVideoId = null; // 當前對應的影片 id
...
export function initMap(containerId) {
    ...
    leafletMap.on('resize', function () { if (markersLayer && markersLayer.getLayers().length > 0)
 ...
 */
export function addMarkersFromMapVisualization(mapVisualization, videoId) {
    // 確保 markersLayer 已建立，盡可能在 leafletMap 或 DOM 已就緒時建立
    if (!markersLayer) {
        if (leafletMap) {
            markersLayer = (typeof L.featureGroup === 'function' ? L.featureGroup() : L.layerGroup()).addTo(leafletMap);
        }
        else if (typeof document !== 'undefined' && document.getElementById && document.getElementById('map')) {
            try {
                initMap('map');
            }
            catch (e) { }
        ...
        if (!markersLayer) {
            markersLayer = (typeof L.featureGroup === 'function' ? L.featureGroup() : L.layerGroup());
        }
    }
    // 清除現有標記並設定目前影片 id
    clearMarkers();
    ...
    function createIconForRoute(route) {
        const type = (route && route.marker) || (route && route.tags && route.tags[0]) || 'default';
        const COLOR_MAP = { Restaurant: '#e74c3c', default: '#2c3e50', Park: '#27ae60', Museum: '#8e44ad', Shop: '#f39c12' };
        const color = COLOR_MAP[type] || COLOR_MAP.default;
        const EMOJI_MAP = { Restaurant: '🍔', Park: '🌳', Museum: '🏛️', Shop: '🛍️' };
    ...
    routes.forEach((route) => {
            ...
            if (marker && typeof marker.on === 'function')
                marker.on('popupopen', function (e) {
                    try {
                        ...
                        function bindOpenLink(el, dataAttr) {
                            if (!el)
                                return;
                            if (el.__tt_bound)
                                return;
        ...
        try {
            console.debug('Added marker', { location: route.location, coords: [lat, lon], markerType: route.marker, tags: route.tags });
        }
        catch (e) { }
    });
    // 確保 markersLayer 已加入地圖（某些情況下 markersLayer 尚未被 addTo）
    try {
        if (markersLayer && leafletMap && (!markersLayer._map || markersLayer._map !== leafletMap)) {
            try {
        ...
        if (bounds) {
            const tryFit = (attemptsLeft = 3, delay = 120) => {
                try {
                    ...
                    if (attemptsLeft > 0)
                        setTimeout(() => tryFit(attemptsLeft - 1, Math.min(600, delay * 2)), delay);
                    else
 ...
 */
export function getMarkersBounds() { try {
    if (markersLayer && typeof markersLayer.getBounds === 'function' && typeof markersLayer.getLayers === 'function' && markersLayer.getLayers().length > 0)
 ...
 */
export function getMarkerLatLngs() { try {
    if (!markersLayer || typeof markersLayer.getLayers !== 'function')
        return [];
    return markersLayer.getLayers().map((l) => { try {
...
try {
    if (typeof window !== 'undefined') {
        ...
        if (!window.initMap)
            window.initMap = window.TrailTag.Map.initMap;
        if (!window.clearMarkers)
            window.clearMarkers = window.TrailTag.Map.clearMarkers;
        if (!window.addMarkersFromMapVisualization)
            window.addMarkersFromMapVisualization = window.TrailTag.Map.addMarkersFromMapVisualization;
