...
function getConfigSync() {
    ...
    if (typeof TRAILTAG_CONFIG !== 'undefined')
        return TRAILTAG_CONFIG;
    // 瀏覽器環境：window 可能含有設定
 ...
 */
export async function fetchWithRetry(url, options = {}, retries = (typeof TRAILTAG_CONFIG !== 'undefined' ? TRAILTAG_CONFIG.FETCH_RETRIES : 2), backoffMs = (typeof TRAILTAG_CONFIG !== 'undefined' ? TRAILTAG_CONFIG.FETCH_BACKOFF_MS : 500)) {
    let attempt = 0;
    while (true) {
        try {
            ...
            if (!res.ok) {
                ...
                try {
                    ...
                    if (parsed && parsed.debug_id)
                        err.debugId = parsed.debug_id;
