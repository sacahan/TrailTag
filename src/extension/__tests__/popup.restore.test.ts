...
describe('popup initializeApp restore/attach', () => {
  ...
  (global as any).window.saveState = async () => { return; };
    const sendMessage = makeMockChrome({ jobId: 'job-from-bg' });

  createDomMocks();
  const popup = await import('../popup');
  const { initializeApp, state: popupState, AppState, queryElements } = popup as any;
  ...
  (global as any).window.loadState = async () => ({ videoId: 'video-2', jobId: 'saved-job', currentState: 'analyzing', progress: 42, phase: 'geocode' });
  (global as any).window.saveState = async () => { return; };
    const sendMessage = makeMockChrome(null);

  const popup = await import('../popup');
  const { initializeApp, state: popupState, queryElements } = popup as any;
  if (typeof queryElements === 'function') queryElements();
