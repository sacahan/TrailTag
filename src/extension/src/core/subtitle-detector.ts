...
import { API_BASE_URL, fetchWithRetry } from "./api";

// 字幕狀態介面定義
export interface SubtitleStatus {
  available: boolean;
  manual_subtitles: string[];
  auto_captions: string[];
...
export class SubtitleChecker {
  ...
  private lastCheckedVideoId: string | null = null;
  private lastCheckResult: SubtitleCheckResult | null = null;

  /**
   * 初始化字幕檢查器
   * @param statusElementId 顯示狀態的 DOM 元素 ID
   */
  constructor(statusElementId: string = "subtitle-status") {
  ...
  async checkSubtitleAvailability(
    ...
    try {
      ...
      const subtitleStatus: SubtitleStatus = await response.json();

      const result: SubtitleCheckResult = {
        success: true,
        status: subtitleStatus,
   ...
   */
  displayResult(result: SubtitleCheckResult, autoHide: number = 0): void {
    if (!this.statusElement) {
    ...
    this.statusElement.innerHTML = `
      <div class="subtitle-status-content">
        <div class="status-message">${result.user_message || "狀態未知"}</div>
        ${
          result.status && result.status.available
            ? '<div class="status-details">可以開始分析此影片</div>'
   ...
   * 獲取上次檢查的結果
   */
  getLastResult(): SubtitleCheckResult | null {
    return this.lastCheckResult;
  }
   ...
   */
  static async quickCheck(videoId: string): Promise<boolean> {
    try {
      const url = `${API_BASE_URL}/api/videos/${videoId}/subtitles/check`;
      const response = await fetchWithRetry(url, {
        method: "GET",
      ...
      const status: SubtitleStatus = await response.json();
